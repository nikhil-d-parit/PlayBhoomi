...
export default function DashboardScreen() {
  ...
  const { bookings, total, loading: dashboardLoading, error } = useSelector((state) => state.dashboard);
  ...
  const loading = dashboardLoading || venuesLoading || usersLoading;
  ...
  const totalTurfsCount = venues.length;
  ...
  const todayEarnings = todayBookings.reduce((sum, b) => sum + (b.amount || 0), 0);
    ...
    const earningsLast7Days = last7Days.map(date => {
      ...
      const pieTotal = totalUsersCount + total;
      const usersPercent = pieTotal ? ((totalUsersCount / pieTotal) * 100).toFixed(1) : 0;
      const bookingsPercent = pieTotal ? ((total / pieTotal) * 100).toFixed(1) : 0;
      const pieChartData = [
        {
          name: `Users (${usersPercent}%)`,
          population: totalUsersCount,
          color: "#2e7d32",
  ...
  loading ? (
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
          ...
          <StatCard icon={Ionicons} label="Total Users" value={totalUsersCount.toString()} />
          <StatCard icon={FontAwesome5} label="Total Turf" value={totalTurfsCount.toString()} />
