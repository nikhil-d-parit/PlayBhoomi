...
import { addVenue } from "../redux/slices/VenuesSlice";
...
const AddVenue = () => {
  ...
  // console.log("Aminities",aminitiesList);
    ...
    sports: [
      {
        ...
        discountedPrice: "",
        weekendPrice: "",
        slotDuration: 60, // Default to 60 minutes (1 hour)
        weekdayTimeSlots: [{ open: "", close: "" }], // Weekday timings (Mon-Fri)
        weekendTimeSlots: [{ open: "", close: "" }], // Weekend timings (Sat-Sun)
        courts: [],
      },
    ],
    amenities: [],
    ...
    if (!form.sports.length) {
      ...
      form.sports.forEach((sport, idx) => {
        ...
        if (!sport.discountedPrice || isNaN(Number(sport.discountedPrice)))
          errors[`sport_${idx}_discountedPrice`] =
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {form.sports.map((sport, si) => (
        <Surface key={si} style={styles.card}>
          ...
          <View style={{ marginTop: 12 }}>
            <Button
              mode="outlined"
              onPress={() => setModal({ type: "courts", sportIndex: si })}
            >
              Select Courts
            </Button>
            <View style={styles.selectedWrap}>
              {sport.courts.length ? (
                sport.courts.map((c) => (
                  <Chip key={c} style={styles.chip} compact>
                    {c}
                  </Chip>
                ))
              ) : (
        ...
        <View style={styles.selectedWrap}>
          {form.amenities.length ? (
            form.amenities.map((id) => {
              ...
              return (
                <Chip key={id} style={styles.chip} compact>
                  {a?.name || id}
                </Chip>
              );
            })
        ...
        <View style={styles.selectedWrap}>
          {form.rules.length ? (
            form.rules.map((id) => {
              ...
              return (
                <Chip key={id} style={[styles.chip, styles.ruleChip]} compact>
                  {r?.name || id}
                </Chip>
              );
            })
          ...
          <Surface style={styles.modalCard}>
            ...
            {courtOptions.map((c) => (
              <View key={c} style={styles.checkboxRow}>
                <Checkbox
                  status={
                    form.sports[modal.sportIndex]?.courts.includes(c)
                      ? "checked"
                      : "unchecked"
                  }
            ...
            <Button onPress={() => setModal({ type: "", sportIndex: null })}>
              Done
            </Button>
          </Surface>
        </View>
      ...
      {/* Rules Modal (different pattern: checkbox list inside card) */}
...
const styles = StyleSheet.create({
  ...
  pickerWrapper: {
    ...
    borderColor: "#d1d5db",
    borderRadius: 8,
    height: 56, // matches TextInput default height
